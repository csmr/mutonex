...
1.  **Builds and Starts Services:** It uses `docker-compose -f ./compose.yaml up -d` to build the Docker images for all services and start them in the background.
...
To run the Docker Compose integration tests on your local machine, navigate to the `./` directory and run:

export class ActionHUD {
  ...
  public setCharmLevel(level: number) {
    ...
    if (
      this.charmValueEl && this.charmValueEl.textContent === val
    ) return;
  ...
  public setNearbyItem(item: { id: string; name: string } | null) {
    if (JSON.stringify(this.nearbyItem) === JSON.stringify(item)) {
      return;
    }
  ...
  public setHoveredItem(item: { id: string; name: string } | null) {
    if (JSON.stringify(this.hoveredItem) === JSON.stringify(item)) {
      return;
    }
  ...
  public setInventory(items: string[]) {
    if (JSON.stringify(this.inventory) === JSON.stringify(items)) {
      return;
    }
  ...
  private render() {
    ...
    // Charm Card
    const charmCard = this.createCard(
      'hud-charm-card',
      'CHARM',
      '0',
    );
    this.charmValueEl = charmCard.querySelector(
      '.card-value',
    ) as HTMLElement;
    ...
    // Nearby Item Card
    if (this.nearbyItem) {
      const pickupCard = this.createCard(
        'hud-pickup-card',
        'PICK UP',
        this.nearbyItem.name,
        '',
        'margin-left: 10px; border-color: #ffd700;',
      );
    ...
    if (
      this.hoveredItem &&
      (!this.nearbyItem ||
        this.hoveredItem.id !== this.nearbyItem.id)
    ) {
      const hoverCard = this.createCard(
        'hud-hover-card',
        'PICK UP',
    ...
    this.inventory.forEach((itemId) => {
      const card = this.createCard(
        ...
        'hud-inventory-card',
        'margin-left: 10px; border-color: #00ff00;',
      );
    ...
    if (!(window as any).hasGlobalHudListeners) {
      window.addEventListener('mousemove', (e) => {
        ...
        if (dy < 0) {
          this.dragTarget.el.style.transform =
            `translateY(${dy}px)`;
        }
      });
      ...
      window.addEventListener('mouseup', (e) => {
        ...
        if (dy < -2 * cardHeight) {
          if (this.onDropClick) {
            this.onDropClick(this.dragTarget.id);
          }
        }

        this.dragTarget.el.style.transition =

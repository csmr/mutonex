...
To generate the `.env` file, `dev-env.sh` startup runs `scripts/init-database-env.sh` and `scripts/init-dotenv.sh`.


## Client pack
See `scripts/build-webclient.sh`. It generates dynamic content, bundles the webclient to `src/dist`, and symlinks `dist` into `gameserver/priv/static` to be served by the backend.

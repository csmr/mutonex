...
import { LobbyView, type Sector } from "./LobbyView.ts";

// A simple map to hold our player avatar meshes
const playerMeshes: Map<string, any> = new Map();
// Map to hold fauna meshes and their target positions for interpolation
const faunaMeshes: Map<string, any> = new Map();
const faunaTargets: Map<string, any> = new Map();
const faunaAnchors: Map<string, any> = new Map(); // Server positions
let scene: any;
...
function main() {
  ...
  lobbyView.show(); // Show lobby initially
  ...
  // --- Game State Handling ---
  let currentTerrainMesh: THREE.Mesh | null = null;
  const onInitialState = (gameState: any) => {
    ...
    currentTerrainMesh = terrainMesh;
  ...
  const onStateUpdate = (update: { players?: PlayerTuple[], fauna?: PlayerTuple[] }) => {
    if (gameStateProvider?.phase === "lobby") {
        // In lobby phase, only update the queue
        if (update.players) lobbyView.updatePlayerQueue(update.players);
    } else {
        // In game phase, hide lobby and update 3D avatars
        lobbyView.hide();
  ...
  lobbyView.onSectorSelect((sector) => {
      ...
      try {
        // Initialize GameStateProvider on selection
        gameStateProvider = new GameStateProvider(onInitialState, onStateUpdate);
        gameStateProvider.start();

        setupAvatarControls(gameStateProvider, camera, controls);
...
function setupAvatarControls(gameStateProvider: GameStateProvider, camera: any, controls: any) {
    // --- Avatar Controls ---
    ...
    function updateLoop() {
        ...
        // Only process game logic if NOT in lobby
...
function updatePlayerAvatars(players: PlayerTuple[]) {
    ...
    for (const playerTuple of players) {
        // Server sends [id, x, y, z]. y is elevation, z is depth.

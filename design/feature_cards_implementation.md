...
*   **Fauna**: Implemented as `Mutonex.Engine.Entities.Fauna`.
    ...
    *   *Note*: Possesses `charm` property and potential for `charm` action, similar to standard Units.
...
*   **Charm**:
    ...
    *   *Description*: All units (including Fauna) possess a `charm` quantity. The `charm` action uses this quantity in a trial against a target's resistance (often their own charm or a Head/Chief's charm).
    *   *Implementation*: `charm` field in Entity structs (`Unit`, `Fauna`, `Building`).
...
3.  **Action System**: Create a system to handle actions like `charm`, `build`, `attack` triggered by clients.

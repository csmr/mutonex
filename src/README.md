...
## Client pack
See `src/scripts/build-webclient.sh`. It generates dynamic content, bundles the webclient to `src/dist`, and symlinks `dist` into `src/gameserver/priv/static` to be served by the backend.

### Content Generation
Game content (models, factions, etc.) is generated by scripts located in `src/res/scripts/`. These scripts are considered source code, while their small generated artifacts are stored in `src/res/` for immediate availability.

(() => {
  ...
  var GameStateProvider = class {
    ...
    start() {
      ...
      this.channel.on("new_token", (payload) => {
  ...
  function createTerrainMesh(terrain, material) {
  ...
  function sampleTerrainHeight(terrainMesh, worldX, worldZ) {
...
`;
  ...
  var LidarView = class {
    ...
    *chunkedGeometryGenerator(samplesH, samplesV) {
    ...
    updateTerrain(terrain) {
      ...
      const material = this.getLidarBaseMaterial(8956552);
  ...
  function main() {
    ...
    featureHUD.setOnCharmClick(() => {
      ...
      const targets = entities.filter((ent) => ent.id !== gameStateProvider.playerId).map((ent) => ({ id: ent.id, dist: avatar.position.distanceTo(ent.pos) })).filter((t) => t.dist <= 20).sort((a, b) => a.dist - b.dist);
      if (targets.length > 0) {
